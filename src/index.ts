...
export const parseAssessmentItemRefsFromXml = (
  xml: string
): AssessmentItemRefParseResult => {
  const doc = parseXml(xml);
  const root = doc.documentElement;
  if (!root) {
...
export const parseResultItemRefsFromXml = (
  xml: string
): ResultItemRefParseResult => {
  const doc = parseXml(xml);
  const root = doc.documentElement;
  if (!root) {
  ...
  for (const itemResult of itemResultNodes) {
    ...
    const parsedSequenceIndex = rawSequenceIndex
      ? Number(rawSequenceIndex)
      : NaN;
    const sequenceIndex =
      hasSequenceIndex &&
      Number.isInteger(parsedSequenceIndex) &&
      parsedSequenceIndex > 0
        ? parsedSequenceIndex
        : null;
...
export const parseResultsXmlRaw = (xml: string): QtiRawResults => {
  ...
  for (const itemResult of itemResultNodes) {
    const identifier = itemResult.getAttribute("identifier") ?? "";
    const sequenceIndex = parsePositiveInteger(
      itemResult.getAttribute("sequenceIndex")
    );

    const responseVariables: Record<string, string[]> = {};
    const responseVarNodes = getElementsByLocalName(
      itemResult,
      "responseVariable"
    );
    for (const rv of responseVarNodes) {
      const rvId = rv.getAttribute("identifier") ?? "";
      const candidateResponse = getElementsByLocalName(
        rv,
        "candidateResponse"
      )[0];
      if (!rvId || !candidateResponse) continue;
      const values = getElementsByLocalName(candidateResponse, "value").map(
        (v) => v.textContent ?? ""
      );
    ...
    const outcomeVarNodes = getElementsByLocalName(
      itemResult,
      "outcomeVariable"
    );
    for (const ov of outcomeVarNodes) {
      ...
      if (!ovId) continue;
      const values = getElementsByLocalName(ov, "value").map(
        (v) => v.textContent ?? ""
      );
